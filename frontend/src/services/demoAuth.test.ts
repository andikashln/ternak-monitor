...
describe('static demo authentication', () => {
  it.each([
    ['owner@dutaagrinusantara.farm', 'OWNER'],
    ['manager@dutaagrinusantara.farm', 'MANAGER'],
    ['akuntan@dutaagrinusantara.farm', 'ACCOUNTANT'],
    ['mitra@dutaagrinusantara.farm', 'MITRA'],
  ...
  it('returns a one-click owner session without exposing credentials to the UI', () => {
    expect(getOneClickDemoSession()).toMatchObject({ user: { role: 'OWNER', email: 'owner@dutaagrinusantara.farm' } });
  });
  ...
  it('restores a documented demo session from its persisted token', () => {
    expect(getStaticDemoSessionFromToken('static-demo-mitra-session')).toMatchObject({
      ...
      user: { email: 'mitra@dutaagrinusantara.farm', role: 'MITRA' },
    });
  ...
  it('rejects invalid credentials instead of bypassing authentication', () => {
    expect(getStaticDemoSession('owner@dutaagrinusantara.farm', 'salah')).toBeNull();
    expect(getStaticDemoSession('unknown@dutaagrinusantara.farm', 'Demo123!')).toBeNull();
  });
