      ...
      <section className="overflow-hidden rounded-3xl bg-[#123b2b] p-6 text-white shadow-xl sm:p-8">
        <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          <div className="max-w-2xl">
            <div className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-emerald-200">
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-xs font-semibold text-emerald-100">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="flex items-center gap-2 text-base font-black text-slate-900">
              <Beef className="h-5 w-5 text-emerald-800" /> Sapi Tersedia
            </h2>
          ...
          <div className="relative w-full sm:max-w-sm">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-300 py-2.5 pl-9 pr-3 text-xs outline-none focus:border-emerald-800 focus:ring-4 focus:ring-emerald-800/10"
            />
          </div>
        </div>
          ...
          {filteredLivestock.map(item => {
            ...
            return (
              <article key={item.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white transition hover:-translate-y-0.5 hover:shadow-lg">
                <div className="relative h-52 bg-gradient-to-br from-emerald-50 to-slate-100">
                  {item.photoUrl ? (
                  ...
                  <span className="absolute left-3 top-3 rounded-full bg-white/95 px-2.5 py-1 text-[10px] font-black text-emerald-900 shadow">
                    {item.healthStatus}
                  </span>
                </div>
                <div className="space-y-3 p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="rounded-lg bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-800">TERSEDIA</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-xs text-slate-600">
                    <span className="flex items-center gap-1.5"><Scale className="h-3.5 w-3.5 text-emerald-800" /> {item.currentWeightKg} kg</span>
                    <span className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5 text-emerald-800" /> {item.locationName}</span>
                  ...
                  <div className="border-t border-slate-100 pt-3">
                    ...
                    <p className="mt-0.5 font-mono text-xl font-black text-emerald-800">{formatRupiah(sellingPrice)}</p>
                  </div>
                </div>
              </article>
            );
