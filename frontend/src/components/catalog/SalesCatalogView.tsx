      ...
      <section className="overflow-hidden rounded-3xl bg-[#123b2b] p-6 text-white shadow-xl sm:p-8">
        <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          <div className="max-w-2xl">
            <div className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-[#EEF2F0]">
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-xs font-semibold text-[#F7F9F8]">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="flex items-center gap-2 text-base font-black text-slate-900">
              <Beef className="h-5 w-5 text-[#0F5132]" /> Sapi Tersedia
            </h2>
          ...
          <div className="relative w-full sm:max-w-sm">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-300 py-2.5 pl-9 pr-3 text-xs outline-none focus:border-[#0F5132] focus:ring-4 focus:ring-[#0F5132]/10"
            />
          </div>
        </div>
          ...
          {filteredLivestock.map(item => {
            ...
            return (
              <article key={item.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white transition hover:-translate-y-0.5 hover:shadow-lg">
                <div className="relative h-52 bg-gradient-to-br from-[#FFFFFF] to-slate-100">
                  {item.photoUrl ? (
                    <img src={item.photoUrl} alt={`Sapi ${item.tagId}`} className="h-full w-full object-cover" />
                  ) : (
                    <div className="flex h-full flex-col items-center justify-center text-[#0A3D26]/45">
                      <Beef className="h-14 w-14" />
                      <span className="mt-2 text-xs font-bold">Foto sapi belum tersedia</span>
                    </div>
                  )}
                  <span className="absolute left-3 top-3 rounded-full bg-white/95 px-2.5 py-1 text-[10px] font-black text-[#0A3D26] shadow">
                    {item.healthStatus}
                  </span>
                </div>
                <div className="space-y-3 p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="rounded-lg bg-[#FFFFFF] px-2 py-1 text-[10px] font-bold text-[#0F5132]">TERSEDIA</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-xs text-slate-600">
                    <span className="flex items-center gap-1.5"><Scale className="h-3.5 w-3.5 text-[#0F5132]" /> {item.currentWeightKg} kg</span>
                    <span className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5 text-[#0F5132]" /> {item.locationName}</span>
                  ...
                  <div className="border-t border-slate-100 pt-3">
                    ...
                    <p className="mt-0.5 font-mono text-xl font-black text-[#0F5132]">{formatRupiah(sellingPrice)}</p>
                  </div>
                </div>
              </article>
            );
