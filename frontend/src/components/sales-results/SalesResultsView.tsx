    ...
    <div className="space-y-4 pb-24 animate-fade-in md:pb-12">
      <header className="rounded-2xl border border-slate-200 bg-white p-4 shadow-2xs sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-3">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#FFFFFF] text-[#1B5E20]"><BadgeDollarSign className="h-5 w-5" /></span>
            <div><h2 className="text-lg font-bold tracking-tight text-slate-950">Hasil Penjualan</h2><p className="mt-1 text-xs leading-relaxed text-slate-500">Pantau omzet, HPP, dan laba bersih setiap invoice ternak.</p></div>
          </div>
          <div className="grid gap-2 sm:grid-cols-[minmax(13rem,1fr)_auto]">
            <label className="relative block"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={searchQuery} onChange={event => setSearchQuery(event.target.value)} placeholder="Cari invoice atau pembeli..." className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-3 text-xs outline-none focus:border-[#1B5E20] focus:ring-4 focus:ring-[#1B5E20]/10" /></label>
            <label className="flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3"><CalendarDays className="h-4 w-4 text-[#1B5E20]" /><input type="month" value={selectedPeriod} onChange={event => setSelectedPeriod(event.target.value || new Date().toISOString().slice(0, 7))} className="bg-transparent text-xs font-bold text-slate-800 outline-none" aria-label="Pilih periode hasil penjualan" /></label>
          </div>
        </div>
      </header>
      ...
      <section className="grid grid-cols-2 gap-2.5 lg:grid-cols-4">
        {[
          ...
          return <article key={metric.label} className="rounded-2xl border border-slate-200 bg-white p-3.5 shadow-2xs sm:p-4"><div className="flex items-start justify-between gap-2"><span className="text-[9px] font-bold uppercase tracking-wider text-slate-500 sm:text-[10px]">{metric.label}</span><span className={`rounded-lg p-1.5 ${metric.tone}`}><Icon className="h-4 w-4" /></span></div><strong className={`mt-3 block break-all font-mono text-sm font-bold sm:text-lg ${metric.label === 'Laba Bersih' && metric.value < 0 ? 'text-rose-700' : 'text-slate-950'}`}>{metric.displayValue ?? formatRupiah(metric.value)}</strong></article>;
        })}
      </section>

      <section className="space-y-3">
        <div className="flex items-end justify-between gap-3 px-1"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#1B5E20]">Rincian per invoice</p><h3 className="mt-1 text-sm font-bold capitalize text-slate-950">Periode {periodLabel}</h3></div><span className="text-[10px] font-bold text-slate-500">{periodSales.length} transaksi</span></div>
        ...
        {periodSales.length === 0 && <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center"><BadgeDollarSign className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 text-sm font-bold text-slate-700">Belum ada hasil penjualan</p><p className="mt-1 text-xs text-slate-400">Tidak ada invoice yang sesuai dengan periode atau pencarian ini.</p></div>}
      </section>
    </div>
  );
};
