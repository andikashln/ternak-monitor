    ...
    <div className="space-y-4 pb-24 animate-fade-in md:pb-12">
      <header className="rounded-2xl border border-slate-200 bg-white p-4 shadow-2xs sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-3">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#FFFFFF] text-[#0F5132]"><BadgeDollarSign className="h-5 w-5" /></span>
          ...
          <div className="grid gap-2 sm:grid-cols-[minmax(13rem,1fr)_auto]">
            <label className="relative block"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={searchQuery} onChange={event => setSearchQuery(event.target.value)} placeholder="Cari invoice atau pembeli..." className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-3 text-xs outline-none focus:border-[#0E9F6E] focus:ring-4 focus:ring-[#0E9F6E]/10" /></label>
            <label className="flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3"><CalendarDays className="h-4 w-4 text-[#0E9F6E]" /><input type="month" value={selectedPeriod} onChange={event => setSelectedPeriod(event.target.value || new Date().toISOString().slice(0, 7))} className="bg-transparent text-xs font-bold text-slate-800 outline-none" aria-label="Pilih periode hasil penjualan" /></label>
          </div>
        </div>
      </header>
      ...
      <section className="grid grid-cols-2 gap-2.5 lg:grid-cols-4">
        {[
          ...
          { label: 'Total Penjualan', value: summary.revenue, icon: TrendingUp, tone: 'bg-[#FFFFFF] text-[#0F5132]' },
      ...
      <section className="space-y-3">
        <div className="flex items-end justify-between gap-3 px-1"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#0E9F6E]">Rincian per invoice</p><h3 className="mt-1 text-sm font-black capitalize text-slate-950">Periode {periodLabel}</h3></div><span className="text-[10px] font-bold text-slate-500">{periodSales.length} transaksi</span></div>
        {periodSales.map(sale => {
          ...
          return (
            <article key={sale.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xs">
              <div className="border-b border-slate-100 bg-slate-50/70 p-4"><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-sm font-black text-slate-950">{sale.invoiceNo}</span><span className={`rounded-full px-2 py-1 text-[9px] font-black ${sale.paymentStatus === 'Lunas' ? 'bg-[#F7F9F8] text-[#0F5132]' : sale.paymentStatus === 'DP' ? 'bg-amber-100 text-amber-800' : 'bg-rose-100 text-rose-700'}`}>{sale.paymentStatus}</span></div><p className="mt-1 text-[10px] text-slate-500">{formatDate(sale.date)} · {sale.buyerName} · {sale.livestockIds.length} ekor</p></div>
              <div className="grid grid-cols-2 gap-px bg-slate-200 sm:grid-cols-3">
                {[
                  ['Penjualan', sale.priceTotal, 'text-[#0F5132]'],
                  ['HPP', hpp, 'text-amber-800'],
                  ['Laba Bersih', netProfit, netProfit >= 0 ? 'text-[#0A3D26]' : 'text-rose-700'],
