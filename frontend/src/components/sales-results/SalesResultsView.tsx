  ...
  return (
    <div className="space-y-4 pb-24 animate-fade-in md:pb-12">
      ...
      <section className="space-y-3">
        <div className="flex items-end justify-between gap-3 px-1"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#1B5E20]">Rincian per invoice</p><h3 className="mt-1 text-sm font-black capitalize text-slate-950">Periode {periodLabel}</h3></div><span className="text-[10px] font-bold text-slate-500">{periodSales.length} transaksi</span></div>
        {periodSales.map(sale => {
          ...
          return (
            <article key={sale.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xs">
              <div className="border-b border-slate-100 bg-slate-50/70 p-4"><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-sm font-black text-slate-950">{sale.invoiceNo}</span><span className={`rounded-full px-2 py-1 text-[9px] font-black ${sale.paymentStatus === 'Lunas' ? 'bg-[#F8FAFC] text-[#1B5E20]' : sale.paymentStatus === 'DP' ? 'bg-amber-100 text-amber-800' : 'bg-rose-100 text-rose-700'}`}>{sale.paymentStatus}</span></div><p className="mt-1 text-[10px] text-slate-500">{formatDate(sale.date)} · {sale.buyerName} · {sale.livestockIds.length} ekor</p></div>
