      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
            <FileSpreadsheet className="w-5 h-5 text-[#0F5132]" />
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2 bg-[#0A3D26] hover:bg-[#0F5132] text-white text-xs font-bold rounded-xl transition cursor-pointer"
        >
          ...
          <div key={rpt.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
              ...
              <div className="flex items-center gap-2">
                <span className={`px-2.5 py-0.5 rounded text-[11px] font-bold ${
                  rpt.reportStatus === 'Disetujui' ? 'bg-[#F7F9F8] text-[#0A3D26]' : 'bg-blue-100 text-blue-900'
                }`}>
                ...
                {storeService.currentUser.role === 'OWNER' && rpt.reportStatus !== 'Disetujui' && (
                  <button
                    ...
                    className="px-3 py-1 bg-[#0F5132] hover:bg-[#0A3D26] text-white text-xs font-bold rounded-lg transition cursor-pointer"
                  >
                    ✓ Setujui Laporan
                  </button>
                )}
            ...
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 text-xs font-mono space-y-2">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-slate-800 font-bold">
                ...
                <div>+ Masuk (Beli/Lahir/Tf): <span className="text-[#0E9F6E]">+{rpt.popPurchase + rpt.popBirth + rpt.popTransferIn}</span></div>
                <div>- Keluar (Jual/Mati/Tf): <span className="text-rose-600">-{rpt.popSales + rpt.popDeath + rpt.popTransferOut}</span></div>
                <div className="text-[#0A3D26] font-black">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs text-slate-700">
              <div>
                ...
                <p>Sehat: <span className="font-bold text-[#0E9F6E]">{rpt.healthyCount}</span> | Sakit: <span className="font-bold text-rose-600">{rpt.sickCount}</span> | Isolasi: {rpt.isolationCount}</p>
              </div>
              <div>
            ...
            <div className="text-[10px] text-slate-400 pt-2 border-t border-slate-100 flex items-center justify-between">
              ...
              {rpt.reviewedBy && <span>Disetujui oleh: <strong className="text-[#0F5132]">{rpt.reviewedBy}</strong> ({formatDate(rpt.reviewedAt)})</span>}
            </div>
          </div>
        ))}
      </div>
            ...
            <form onSubmit={handleSaveReport} className="p-5 space-y-3 text-xs max-h-[85vh] overflow-y-auto">
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  ...
                  <div>
                    <label className="block text-[10px] text-[#0E9F6E] font-semibold">+ Pembelian</label>
                    <input
                      type="number"
                      value={popPurchase}
                  ...
                  <div>
                    <label className="block text-[10px] text-[#0E9F6E] font-semibold">+ Kelahiran</label>
                    <input
                      type="number"
                      value={popBirth}
                  ...
                  <div>
                    <label className="block text-[10px] text-[#0E9F6E] font-semibold">+ Transfer In</label>
                    <input
                      type="number"
                      value={popTransferIn}
                ...
                <div className="p-2 bg-[#F7F9F8]/80 rounded text-center text-xs font-bold text-[#0A3D26] font-mono">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#0F5132]"
                />
              </div>
              <div>
