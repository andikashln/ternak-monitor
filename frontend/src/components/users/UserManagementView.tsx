      ...
      <div className="flex flex-col justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-2xs sm:flex-row sm:items-center">
        <div>
          <h2 className="flex items-center gap-2 text-lg font-bold text-slate-900">
            <Users className="h-5 w-5 text-[#0F5132]" /> Manajemen Pengguna
          </h2>
          <p className="text-xs text-slate-500">Kelola akun, role, lokasi akses, status, dan password pengguna.</p>
        </div>
        <button onClick={() => { setMessage(null); setIsCreateOpen(true); }} className="flex items-center justify-center gap-2 rounded-xl bg-[#0A3D26] px-4 py-2.5 text-xs font-bold text-white hover:bg-[#0F5132]">
          <Plus className="h-4 w-4" /> Buat Akun
        </button>
      </div>

      {message && (
        <div className={`flex items-center gap-2 rounded-xl border p-3 text-xs font-semibold ${message.type === 'success' ? 'border-[#EEF2F0] bg-[#FFFFFF] text-[#0A3D26]' : 'border-rose-200 bg-rose-50 text-rose-800'}`}>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-2xs">
        <div className="flex flex-col gap-3 border-b border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative max-w-sm flex-1">
            ...
            <input value={search} onChange={event => setSearch(event.target.value)} placeholder="Cari nama, email, atau role..." className="w-full rounded-lg border border-slate-300 py-2 pl-9 pr-3 text-xs outline-none focus:ring-2 focus:ring-[#0F5132]" />
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredUsers.map(user => (
                  <tr key={user.uid} className="hover:bg-slate-50/70">
                    <td className="p-4"><div className="font-bold text-slate-900">{user.displayName}{user.uid === currentUser.uid && <span className="ml-2 text-[9px] text-[#0E9F6E]">ANDA</span>}</div><div className="text-slate-500">{user.email}{user.phone ? ` · ${user.phone}` : ''}</div></td>
                    <td className="p-4">
                      {canManage(user) ? (
                      ...
                      ) : <span className="rounded-full bg-[#F7F9F8] px-2.5 py-1 font-bold text-[#0A3D26]">{roleLabels[user.role]}</span>}
                    </td>
                    ...
                    <td className="p-4"><span className={`rounded-full px-2.5 py-1 font-bold ${user.status === 'Aktif' ? 'bg-[#F7F9F8] text-[#0A3D26]' : 'bg-slate-200 text-slate-600'}`}>{user.status}</span></td>
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between bg-[#0A3D26] p-4 text-white"><h3 className="flex items-center gap-2 font-bold"><ShieldCheck className="h-5 w-5" /> Buat Akun Pengguna</h3><button onClick={() => setIsCreateOpen(false)}><X className="h-5 w-5" /></button></div>
            <form onSubmit={handleCreate} className="space-y-4 p-5 text-xs">
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="font-bold text-slate-700">Nama Lengkap *<input required value={form.displayName} onChange={event => setForm({ ...form, displayName: event.target.value })} className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 font-normal outline-none focus:ring-2 focus:ring-[#0F5132]" /></label>
                <label className="font-bold text-slate-700">Nomor Telepon<input value={form.phone} onChange={event => setForm({ ...form, phone: event.target.value })} className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 font-normal outline-none focus:ring-2 focus:ring-[#0F5132]" /></label>
              </div>
              <label className="block font-bold text-slate-700">Email *<input type="email" required value={form.email} onChange={event => setForm({ ...form, email: event.target.value })} className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 font-normal outline-none focus:ring-2 focus:ring-[#0F5132]" /></label>
              <label className="block font-bold text-slate-700">Password Sementara *<input type="password" minLength={8} required value={form.password} onChange={event => setForm({ ...form, password: event.target.value })} className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 font-normal outline-none focus:ring-2 focus:ring-[#0F5132]" /><span className="mt-1 block font-normal text-slate-400">Minimal 8 karakter. Sampaikan secara aman kepada pengguna.</span></label>
              <label className="block font-bold text-slate-700">Role *<select value={form.role} onChange={event => setForm({ ...form, role: event.target.value as UserRole })} className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 font-normal outline-none focus:ring-2 focus:ring-[#0F5132]">{allowedCreateRoles.map(role => <option key={role} value={role}>{roleLabels[role]}</option>)}</select></label>
              <fieldset><legend className="font-bold text-slate-700">Akses Lokasi</legend><p className="mt-1 text-slate-400">Tidak memilih lokasi berarti akses ke semua lokasi.</p><div className="mt-2 grid gap-2 sm:grid-cols-2">{locations.map(location => <label key={location.id} className="flex items-center gap-2 rounded-lg border border-slate-200 p-2.5 font-semibold text-slate-700"><input type="checkbox" checked={form.locationIds.includes(location.id)} onChange={() => toggleLocation(location.id)} className="accent-[#0F5132]" />{location.name}</label>)}</div></fieldset>
              <div className="flex justify-end gap-2 border-t border-slate-100 pt-4"><button type="button" onClick={() => setIsCreateOpen(false)} className="rounded-lg border border-slate-300 px-4 py-2 font-bold text-slate-600">Batal</button><button disabled={saving} className="flex items-center gap-2 rounded-lg bg-[#0A3D26] px-5 py-2 font-bold text-white disabled:opacity-60">{saving && <Loader2 className="h-4 w-4 animate-spin" />}Simpan Akun</button></div>
            </form>
          </div>
        </div>
      )}
      ...
      {resetTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
          <form onSubmit={handleResetPassword} className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-2xl">
            ...
            <input type="password" minLength={8} required autoFocus value={newPassword} onChange={event => setNewPassword(event.target.value)} placeholder="Minimal 8 karakter" className="mt-4 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#0F5132]" />
