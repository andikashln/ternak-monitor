...
import { AgroHeader, AgroStat, AgroTable, StatusBadge, AgroButton, AgroSearch, AgroModal, AgroField, AgroSelect, AgroEmpty, AddButton } from './AgroUI';

const emptyDraft = (): Omit<CropRecord, 'id' | 'createdAt' | 'updatedAt'> => ({
  name: '', division: 'Sayuran Jangka Pendek', variety: '', locationId: '', locationName: '',
  plotAreaM2: 0, plantedDate: new Date().toISOString().slice(0, 10), estimatedHarvestDate: '', status: 'Tanam', notes: '',
});

// Sayuran jangka pendek (kangkung, bayam, cabai, dll).
export const CropShortTermView: React.FC = () => {
  ...
  const crops = state.crops.filter(c => c.division === 'Sayuran Jangka Pendek');
  ...
  return (
    <div className="space-y-5 pb-16">
      <AgroHeader
        ...
        title="Sayuran Jangka Pendek"
        subtitle="Kelola tanaman hortikultura dengan siklus panen singkat."
      />
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <AgroStat label="Siap Panen" value={String(crops.filter(c => c.status === 'Panen').length)} accent />
      </div>
      ...
      {filtered.length === 0 ? (
        <AgroEmpty text="Belum ada sayuran jangka pendek." />
      ) : (
        <AgroTable headers={['Nama', 'Varietas', 'Lokasi', 'Luas (m²)', 'Tanggal Tanam', 'Estimasi Panen', 'Status', 'Aksi']}>
          {filtered.map(c => (
            <tr key={c.id} className="hover:bg-slate-50">
              <td className="px-4 py-3 font-bold text-[#1B5E20]">{c.name}</td>
      ...
      {showForm && (
        <AgroModal title={editingId ? 'Edit Tanaman' : 'Tambah Tanaman'} onClose={() => setShowForm(false)}>
          <AgroField label="Nama Tanaman" value={draft.name} onChange={v => setDraft({ ...draft, name: v })} placeholder="Kangkung" />
          <div className="grid grid-cols-2 gap-3">
            <AgroField label="Varietas" value={draft.variety} onChange={v => setDraft({ ...draft, variety: v })} placeholder="Kangkung Bangkok" />
            <AgroField label="Nama Lokasi" value={draft.locationName} onChange={v => setDraft({ ...draft, locationName: v })} placeholder="Sontang" />
          </div>
          <div className="grid grid-cols-2 gap-3">
