...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, setActiveTab, onOpenQuickAction }) => (
  <nav aria-label="Navigasi cepat" className="app-bottom-nav fixed inset-x-0 bottom-0 z-40 border-t border-[#5a2d1f]/15 bg-white/95 px-2 pt-1.5 shadow-[0_-12px_30px_rgba(15,50,38,0.1)] backdrop-blur-xl lg:hidden">
    <div className="mx-auto grid max-w-lg grid-cols-5 items-end">
      ...
      <button type="button" onClick={onOpenQuickAction} className="group flex min-h-14 flex-col items-center justify-end gap-1 text-[10px] font-black text-[#5a2d1f]" aria-label="Tambah catatan baru">
        <span className="flex h-12 w-12 -translate-y-2 items-center justify-center rounded-2xl bg-[#5a2d1f] text-white shadow-lg shadow-[#5a2d1f]/20 ring-4 ring-[#fff8e8] transition active:scale-95"><Plus className="h-6 w-6" /></span>
