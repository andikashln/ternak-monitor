...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, setActiveTab, onOpenQuickAction, role }) => {
  ...
  const renderItem = (item: (typeof items)[number]) => {
    ...
    return <button key={item.id} type="button" onClick={() => setActiveTab(item.id)} aria-current={isActive ? 'page' : undefined} className={`flex min-h-14 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-bold transition ${isActive ? 'text-[#0F5132]' : 'text-slate-400'}`}><span className={`rounded-lg p-1 ${isActive ? 'bg-[#EEF2F0]' : ''}`}><Icon className="h-5 w-5" /></span><span>{item.label}</span></button>;
  };
  return <nav aria-label="Navigasi cepat" className="app-bottom-nav fixed inset-x-0 bottom-0 z-40 border-t border-[#0F5132]/15 bg-white/95 px-2 pt-1.5 shadow-[0_-12px_30px_rgba(15,50,38,0.1)] backdrop-blur-xl lg:hidden">
    <div className="mx-auto grid max-w-lg grid-cols-5 items-end">
      {items.slice(0, splitIndex).map(renderItem)}
      <button type="button" onClick={onOpenQuickAction} className="group flex min-h-14 flex-col items-center justify-end gap-1 text-[10px] font-black text-[#0F5132]" aria-label="Tambah catatan baru"><span className="flex h-12 w-12 -translate-y-2 items-center justify-center rounded-2xl bg-[#0F5132] text-white shadow-lg shadow-[#0F5132]/20 ring-4 ring-[#FFFFFF] transition active:scale-95"><Plus className="h-6 w-6" /></span><span className="-mt-2">Tambah</span></button>
