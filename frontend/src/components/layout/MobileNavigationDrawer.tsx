      ...
      <aside className="drawer-enter absolute inset-y-0 left-0 flex w-[min(88vw,22rem)] flex-col bg-[#fbfdfb] shadow-2xl">
        ...
        <div className="border-b border-slate-100 px-4 py-3">
          <label className="relative block">
            ...
            <input
              ...
              className="h-11 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-3 text-sm outline-none focus:border-emerald-700 focus:ring-4 focus:ring-emerald-700/10"
            />
          </label>
        </div>
              ...
              <div key={section.label} className="mb-5">
                ...
                <div className="space-y-1">
                  {visibleItems.map(item => {
                    ...
                    return (
                      <button key={item.id} type="button" onClick={() => handleNavigate(item.id)} className={`flex min-h-12 w-full items-center gap-3 rounded-xl px-3 py-2 text-left ${isActive ? 'bg-emerald-900 text-white shadow-sm' : 'text-slate-700 hover:bg-emerald-50'}`}>
                        <Icon className={`h-5 w-5 shrink-0 ${isActive ? 'text-emerald-100' : 'text-emerald-800'}`} />
                        <span className="min-w-0 flex-1">
                          <span className="block text-sm font-extrabold">{item.label}</span>
                          <span className={`block text-[11px] ${isActive ? 'text-emerald-100/75' : 'text-slate-400'}`}>{item.description}</span>
                        </span>
        ...
        <div className="border-t border-slate-200 bg-white p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
          <div className="mb-3 flex items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-100 text-xs font-black text-emerald-900">{currentUser.displayName.split(' ').map(name => name[0]).join('').slice(0, 2).toUpperCase()}</span>
            <span className="min-w-0 flex-1">
