  ...
  return (
    <div className="space-y-3 pb-24 animate-fade-in sm:space-y-4 md:pb-12">
      <header className="ranch-panel rounded-2xl border p-4 sm:p-5">
        <div>
          <h2 className="ranch-heading flex items-center gap-2 text-lg font-bold">
            <Wallet className="h-5 w-5 text-[#0F5132]" />
      ...
      <section className="ranch-panel flex flex-col gap-3 rounded-2xl border p-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#0E9F6E]">Ringkasan {granularity === 'month' ? 'bulanan' : 'tahunan'}</p>
        ...
        <div className="grid w-full grid-cols-2 gap-2 lg:flex lg:w-auto lg:items-center">
          ...
          <button
            ...
            className="flex items-center justify-center gap-1.5 rounded-xl bg-[#0A3D26] px-3.5 py-2.5 text-xs font-bold text-white transition hover:bg-[#0F5132]"
          >
      ...
      <section className="grid grid-cols-2 gap-2.5 sm:grid-cols-4 sm:gap-3">
        <article className="rounded-2xl border border-[#EEF2F0] bg-white p-4 shadow-2xs sm:p-5">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-wider text-slate-500">Pemasukan</span>
            <ArrowUpRight className="h-4 w-4 text-[#0E9F6E]" />
          </div>
          <strong className="mt-2 block font-mono text-lg font-black text-[#0F5132]">{formatRupiah(totalIncome)}</strong>
        ...
        <article className={`rounded-2xl border bg-white p-4 shadow-2xs sm:p-5 ${grossProfit >= 0 ? 'border-[#EEF2F0]' : 'border-rose-200'}`}>
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-wider text-slate-500">Laba Kotor</span>
            <span className={`rounded-full px-2 py-1 text-[9px] font-black ${grossProfit >= 0 ? 'bg-[#FFFFFF] text-[#0F5132]' : 'bg-rose-50 text-rose-700'}`}>{grossProfit >= 0 ? 'LABA' : 'RUGI'}</span>
          </div>
          <strong className={`mt-2 block font-mono text-lg font-black ${grossProfit >= 0 ? 'text-[#0F5132]' : 'text-rose-600'}`}>{formatRupiah(grossProfit)}</strong>
          <p className="mt-1 text-[10px] text-slate-400">Penjualan dikurangi HPP ternak</p>
        </article>

        <article className={`rounded-2xl border bg-white p-4 shadow-2xs sm:p-5 ${netProfit >= 0 ? 'border-[#EEF2F0]' : 'border-rose-200'}`}>
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-wider text-slate-500">Laba Bersih</span>
            <span className={`rounded-full px-2 py-1 text-[9px] font-black ${netProfit >= 0 ? 'bg-[#FFFFFF] text-[#0F5132]' : 'bg-rose-50 text-rose-700'}`}>{netProfit >= 0 ? 'LABA' : 'RUGI'}</span>
          </div>
          <strong className={`mt-2 block font-mono text-lg font-black ${netProfit >= 0 ? 'text-[#0F5132]' : 'text-rose-600'}`}>{formatRupiah(netProfit)}</strong>
          ...
          {categorySummaries.map(item => {
            ...
            return (
              <div key={item.category} className="bg-white p-3.5 sm:p-4">
                ...
                <span className={`mt-2 block font-mono text-sm font-black ${isIncome ? 'text-[#0F5132]' : 'text-rose-600'}`}>
              ...
              <article key={transaction.id} className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <span className={`inline-flex rounded-md px-2 py-1 text-[9px] font-bold ${isIncome ? 'bg-[#FFFFFF] text-[#0F5132]' : 'bg-rose-50 text-rose-700'}`}>
                      {getCategoryLabel(transaction.category)}
                    </span>
                    <p className="mt-2 line-clamp-2 text-xs font-bold leading-relaxed text-slate-800">{transaction.description}</p>
                  </div>
                  <strong className={`shrink-0 whitespace-nowrap font-mono text-sm font-black ${isIncome ? 'text-[#0F5132]' : 'text-rose-600'}`}>
            ...
            <tbody className="divide-y divide-slate-100 font-medium">
              {periodTransactions.map(transaction => (
                <tr key={transaction.id} className="transition hover:bg-slate-50">
                  ...
                  <td className={`whitespace-nowrap p-3.5 text-right font-mono text-sm font-black ${transaction.type === 'income' ? 'text-[#0F5132]' : 'text-rose-600'}`}>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/60 p-0 backdrop-blur-xs sm:items-center sm:p-4">
          <div className="max-h-[94dvh] w-full max-w-lg overflow-y-auto rounded-t-2xl border border-slate-200 bg-white shadow-2xl sm:max-h-[92vh] sm:rounded-2xl">
            <div className="sticky top-0 flex items-center justify-between bg-[#0A3D26] p-4 font-bold text-white">
              <h3>{editingTransactionId ? 'Edit Transaksi' : 'Catat Transaksi'}</h3>
              <button type="button" onClick={() => setIsModalOpen(false)} className="rounded p-1 hover:bg-[#0F5132]" aria-label="Tutup form">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveTransaction} className="space-y-3 p-4 text-xs sm:space-y-4 sm:p-5">
              <div>
                ...
                <select value={category} onChange={event => setCategory(event.target.value as FinancialCategoryType)} className="w-full rounded-lg border border-slate-300 px-3 py-2.5 font-bold outline-none focus:ring-2 focus:ring-[#0F5132]">
                  {categoryOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}
                </select>
                <div className={`mt-2 rounded-lg px-3 py-2 text-[10px] font-semibold ${transactionType === 'income' ? 'bg-[#FFFFFF] text-[#0F5132]' : 'bg-rose-50 text-rose-700'}`}>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <input value={invoiceNo} onChange={event => setInvoiceNo(event.target.value)} required className="w-full rounded-lg border border-slate-300 px-3 py-2 font-mono outline-none focus:ring-2 focus:ring-[#0F5132]" />
                </div>
                <div>
                  <label className="mb-1 block font-bold text-slate-700">Tanggal *</label>
                  <input type="date" value={transactionDate} onChange={event => setTransactionDate(event.target.value)} required className="w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-[#0F5132]" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <input type="number" min="1" value={amount} onChange={event => setAmount(event.target.value)} required placeholder="Contoh: 500000" className="w-full rounded-lg border border-slate-300 px-3 py-2 font-mono font-bold outline-none focus:ring-2 focus:ring-[#0F5132]" />
                </div>
                <div>
                  <label className="mb-1 block font-bold text-slate-700">Lokasi Kandang *</label>
                  <select value={locationId} onChange={event => setLocationId(event.target.value)} required className="w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-[#0F5132]">
              ...
              <div>
                ...
                <input value={description} onChange={event => setDescription(event.target.value)} required placeholder="Jelaskan transaksi secara singkat" className="w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-[#0F5132]" />
              </div>

              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div>
                  <label className="mb-1 block font-bold text-slate-700">Metode Pembayaran</label>
                  <select value={paymentMethod} onChange={event => setPaymentMethod(event.target.value)} className="w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-[#0F5132]">
                ...
                <div>
                  ...
                  <input value={payeePayer} onChange={event => setPayeePayer(event.target.value)} placeholder="Nama pihak terkait" className="w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-[#0F5132]" />
                </div>
              </div>
              ...
              <div className="flex justify-end gap-2 border-t border-slate-100 pt-4">
                ...
                <button type="submit" className="rounded-lg bg-[#0A3D26] px-5 py-2 font-bold text-white hover:bg-[#0F5132]">{editingTransactionId ? 'Simpan Perubahan' : 'Simpan Transaksi'}</button>
              </div>
            </form>
          </div>
        </div>
